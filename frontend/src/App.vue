    ...
    <main>
      ...
      <section v-if="!user" class="auth-grid">
        ...
        <form class="panel auth-form" @submit.prevent="action(authenticate)">
          ...
          <label>Email<input v-model="email" type="email" autocomplete="username" required maxlength="254" placeholder="name@example.test"></label>
          <label>Пароль<input v-model="password" type="password" :autocomplete="registering ? 'new-password' : 'current-password'" required minlength="8" maxlength="64" placeholder="От 8 символов"></label>
      ...
      <section v-else>
        ...
        <form class="panel create-form" aria-label="Новая заявка" @submit.prevent="action(createTicket)"><label class="grow">Заголовок новой заявки<input v-model="title" required minlength="3" maxlength="80" placeholder="Например, не открывается учебный портал"></label><label>Приоритет новой заявки<select v-model="priority"><option v-for="(label, key) in priorityLabels" :key="key" :value="key">{{ label }}</option></select></label><button class="primary" :disabled="busy">Создать заявку</button></form>
