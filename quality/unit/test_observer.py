...
""", encoding="utf-8")
    ...
    process = subprocess.run([sys.executable, "-m", "pytest", "-p", "grader.observer", str(sample), "-q"], env=env, cwd=root, capture_output=True)
    ...
    subprocess.run([sys.executable, "-m", "pytest", "-p", "grader.observer", str(sample), "-k", "test_assertion", "-q"], env=env, cwd=root, capture_output=True)
