...
def test_catalog_reserves_fifteen_assignments_and_links_published_first():
    """Индекс содержит 15 позиций, но файл создан только для опубликованной первой работы."""
    ...
    assert linked == ["01-знакомство-с-архитектурами.md"]
    assert (CATALOG / linked[0]).is_file()
    assert all(label.endswith("— запланировано.") for _, label in items[1:])
